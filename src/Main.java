    ...
    static class LoginValidationException extends Exception {
        ...
        public static void validateLogin(String login) throws LoginValidationException {
            ...
            for (int i = 0; i < login.length(); i++) {
                if ((login.charAt(i) < 65 || login.charAt(i) > 90) && (login.charAt(i) < 97 || login.charAt(i) > 122) && (login.charAt(i) != 95) && (login.charAt(i) < 48 || login.charAt(i) > 57)) {
                    valid2 = false;
                }
            }
            if (!valid2) {
            ...
            int indexA = login.indexOf('_');
            if (indexA == -1) {
                throw new LoginValidationException();
            }

            for (int i = 0; i < login.length(); i++) {
                if (login.charAt(i) > 64 && login.charAt(i) < 91) valid = true;
            }
            if (!valid) {
                throw new LoginValidationException();
            }
            for (int i = 0; i < login.length(); i++) {
                if (login.charAt(i) > 47 && login.charAt(i) < 58) valid3 = true;
            }
            if (!valid3) {
                throw new LoginValidationException();
            }
            ...
            for (int i = 0; i < login.length(); i++) {
                if (login.charAt(i) > 96 && login.charAt(i) < 123) valid1 = true;
            }
            if (!valid1) {
                throw new LoginValidationException();
            }
